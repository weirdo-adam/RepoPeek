        ...
        public func snapshot(
            ...
            func loadStatus(at repoURL: URL) -> LocalRepoStatus? {
                ...
                return LocalRepoStatus(
                    ...
                    name: repoName,
                    fullName: fullName,
                    remoteWebURLHost: remote?.webURLHost,
                    branch: branch,
                    isClean: isClean,
                    aheadCount: ahead,
                    behindCount: behind,
    ...
    private struct GitRemote {
        ...
        private static func parseURL(_ value: String) -> GitRemote? {
            ...
            let parts = self.pathParts(from: url.path)
            guard parts.count >= 2 else { return nil }

            let projectPath = parts.joined(separator: "/")
            let name = parts.last ?? ""
            guard let webURLHost = self.webURLHost(scheme: url.scheme, host: host, port: url.port) else { return nil }

            return GitRemote(webURLHost: webURLHost, projectPath: projectPath, name: name)
        }
        ...
        private static func parseScp(_ value: String) -> GitRemote? {
            ...
            let path = parts[1]
            let pathParts = self.pathParts(from: path)
            guard pathParts.count >= 2 else { return nil }

            let projectPath = pathParts.joined(separator: "/")
            let name = pathParts.last ?? ""
            guard let webURLHost = self.webURLHost(scheme: nil, host: hostPart, port: nil) else { return nil }

            return GitRemote(webURLHost: webURLHost, projectPath: projectPath, name: name)
        }

        private static func pathParts(from value: String) -> [String] {
            var parts = value.split(separator: "/").map(String.init)
            guard parts.isEmpty == false else { return [] }

            let lastIndex = parts.count - 1
            parts[lastIndex] = self.stripGitSuffix(parts[lastIndex])
            return parts.filter { $0.isEmpty == false }
        }

        private static func webURLHost(scheme: String?, host: String, port: Int?) -> URL? {
