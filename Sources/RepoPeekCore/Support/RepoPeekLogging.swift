...
private final class LogState: @unchecked Sendable {
    ...
    private let subsystem = "com.weirdoadam.repopeek"
    ...
    func bootstrapIfNeeded() {
        ...
        LoggingSystem.bootstrap { label in
            RepoPeekLogHandler(label: label, state: self)
        }
    }
    ...
    private func resolveLogFileURL() -> URL? {
        ...
        let directory = base
            .appending(path: RepoPeekProductConstants.cachesDirectoryName, directoryHint: .isDirectory)
            .appending(path: "Logs", directoryHint: .isDirectory)
        return directory.appending(path: "repopeek.log", directoryHint: .notDirectory)
    }
}

private struct RepoPeekLogHandler: LogHandler {
    let label: String
