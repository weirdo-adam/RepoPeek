...
public enum RepoAutocompleteScoring {
    ...
    public static func score(repo: Repository, query: String) -> Int? {
        ...
        if hasSlash {
            return Self.pathScore(fullName: fullName, query: lowerQuery)
        }
    ...
    private static func pathScore(fullName: String, query: String) -> Int? {
        if fullName == query { return 1000 }
        if fullName.hasPrefix(query) { return 700 }

        let queryComponents = query
            .split(separator: "/", omittingEmptySubsequences: false)
            .map(String.init)
        let targetComponents = fullName
            .split(separator: "/", omittingEmptySubsequences: false)
            .map(String.init)

        guard queryComponents.count >= 2,
              targetComponents.count >= 2,
              let finalQuery = queryComponents.last,
              let finalTarget = targetComponents.last,
              !finalQuery.isEmpty else { return nil }

        let prefixQueries = queryComponents.dropLast()
        guard prefixQueries.count <= targetComponents.dropLast().count else { return nil }

        var score = 80
        for (index, queryComponent) in prefixQueries.enumerated() {
            guard !queryComponent.isEmpty else { return nil }

            let targetComponent = targetComponents[index]
            if targetComponent == queryComponent {
                score += 180
            } else if targetComponent.hasPrefix(queryComponent) {
                score += 120
            } else {
                return nil
            }
        ...
        guard let finalScore = Self.componentScore(
            query: finalQuery,
            target: finalTarget,
            weights: ComponentScoreWeights(
                exact: 600,
