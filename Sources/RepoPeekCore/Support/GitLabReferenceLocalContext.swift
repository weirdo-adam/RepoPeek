...
public enum GitLabReferenceLocalContext {
    ...
    public static func queries(
        ...
        var scopedQueries: [GitLabReferenceQuery] = []
    ...
    public static func repositoryFullName(in text: String, localRepoIndex: LocalRepoIndex = .empty) async -> String? {
        ...
        for path in paths {
            ...
            let task = Task.detached(priority: .utility) {
                self.gitLabRepositoryFullName(at: expandedPath)
            }
    ...
    public nonisolated static func gitLabRepositoryFullName(at path: String) -> String? {
        #if os(macOS)
            let process = Process()
            ...
            return self.gitLabRepositoryFullName(fromRemoteURL: remote)
        #else
            nil
        #endif
    }

    public nonisolated static func gitLabRepositoryFullName(fromRemoteURL remote: String) -> String? {
        if remote.contains("://") {
        ...
        let pathParts = self.pathParts(from: parts[1])
        guard pathParts.count >= 2 else { return nil }

        return pathParts.joined(separator: "/")
    }

    private nonisolated static func pathParts(from value: String) -> [String] {
