...
public struct PersistentHTTPResponse: Equatable, Sendable {
    ...
    func httpResponse(for url: URL) -> HTTPURLResponse? {
        HTTPURLResponse(
            ...
            httpVersion: nil,
            headerFields: self.headers.isEmpty ? nil : self.headers
        )
    }
}

public struct RepoPeekCacheSummary: Codable, Equatable, Sendable {
    public let databasePath: String
    public let exists: Bool
    public let apiResponseCount: Int
    public let rateLimitCount: Int
    public let latestResponses: [RepoPeekCachedResponseSummary]
    public let rateLimits: [RepoPeekRateLimitSummary]
}

public struct RepoPeekCachedResponseSummary: Codable, Equatable, Sendable {
...
public struct RepoPeekRateLimitSummary: Codable, Equatable, Sendable {
...
public enum RepoPeekPersistentCache {
    public static func standardDatabaseURL(fileManager: FileManager = .default, accountID: String? = nil) -> URL? {
        HTTPResponseDiskCache.standardDatabaseURL(fileManager: fileManager, accountID: accountID)
    }
    ...
    ) throws -> RepoPeekCacheSummary {
        guard let url = self.standardDatabaseURL(fileManager: fileManager, accountID: accountID) else {
            throw RepoPeekCacheError.missingApplicationSupportDirectory
        }

        let exists = fileManager.fileExists(atPath: url.path)
        guard exists else {
            return RepoPeekCacheSummary(
    ...
    public static func clear(fileManager: FileManager = .default, accountID: String? = nil) throws -> RepoPeekCacheSummary {
        guard let url = self.standardDatabaseURL(fileManager: fileManager, accountID: accountID) else {
            throw RepoPeekCacheError.missingApplicationSupportDirectory
        }
...
public enum RepoPeekCacheError: Error, LocalizedError {
...
/// DatabaseQueue serializes access; GitLab lookup task groups share this cache.
final class HTTPResponseDiskCache: @unchecked Sendable {
    private let queue: DatabaseQueue
    private let path: String
    private let clock: @Sendable () -> Date
    private let logger = RepoPeekLogging.logger("cache-db")
    ...
    static func standard(accountID: String? = nil) -> HTTPResponseDiskCache? {
        guard let path = standardDatabaseURL(accountID: accountID)?.path else { return nil }

        do {
            return try HTTPResponseDiskCache(path: path)
        } catch {
            RepoPeekLogging.logger("cache-db").error("Unable to open persistent cache: \(error.localizedDescription)")
            return nil
        }
    }

    static func standardDatabaseURL(fileManager: FileManager = .default, accountID: String? = nil) -> URL? {
        ...
        guard let accountID = AccountScopedRepositoryLists.normalizedAccountID(accountID) else {
            return appSupport.appending(path: "Cache.sqlite", directoryHint: .notDirectory)
        }

        return appSupport
            .appending(path: "Cache", directoryHint: .isDirectory)
            .appending(path: self.accountCacheFileName(accountID: accountID), directoryHint: .notDirectory)
    }

    private static func accountCacheFileName(accountID: String) -> String {
        let hex = accountID.utf8.map { String(format: "%02x", $0) }.joined()
        return "v2-\(hex).sqlite"
    }
        ...
        do {
            return try self.queue.read { db in
                guard let row = try Row.fetchOne(
                    db,
                    sql: """
                    select etag, body, fetched_at, status_code, headers_json
                    from api_responses
                    where key = ? and etag is not null
                    """,
                    arguments: [key]
                ) else { return nil }
    ...
    func count() -> Int {
        do {
            return try self.queue.read { db in
                try Int.fetchOne(db, sql: "select count(*) from api_responses where etag is not null") ?? 0
            }
        } catch {
    ...
    func summary(limit: Int = 10) throws -> RepoPeekCacheSummary {
        try self.queue.read { db in
            ...
            let responses = try Row.fetchAll(
                ...
                let etag: String? = row["etag"]
                return RepoPeekCachedResponseSummary(
                    method: row["method"],
                    url: row["url"],
            ...
            let rateLimits = try Row.fetchAll(
                ...
                return RepoPeekRateLimitSummary(
    ...
    static func migrate(_ queue: DatabaseQueue) throws {
        ...
        migrator.registerMigration("v2") { _ in }
    ...
    private static func decodeHeaders(_ rawValue: String?) -> [String: String] {
        guard let rawValue,
