...
public enum HeatmapFilter {
    public static func gitLabCalendar(timeZone: TimeZone = .current) -> Calendar {
    ...
    public static func alignedRange(
        ...
        let daysToWeekStart = (startWeekday - calendar.firstWeekday + 7) % 7
        let alignedStart = calendar.date(byAdding: .day, value: -daysToWeekStart, to: start) ?? start
