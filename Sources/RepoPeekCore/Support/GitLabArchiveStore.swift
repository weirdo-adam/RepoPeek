...
public struct GitLabArchiveStatusOutput: Codable, Equatable, Sendable {
    public let sources: [GitLabArchiveSourceStatus]

    public init(sources: [GitLabArchiveSourceStatus]) {
        self.sources = sources
    }
}

public struct GitLabArchiveSourceStatus: Codable, Equatable, Sendable {
    public let id: String
    public let name: String
    public let enabled: Bool
    public let format: GitLabArchiveFormat
...
public struct GitLabArchiveUpdateResult: Codable, Equatable, Sendable {
    public let source: GitLabArchiveSource
    public let importResult: GitLabArchiveImportResult

    public init(source: GitLabArchiveSource, importResult: GitLabArchiveImportResult) {
        self.source = source
        self.importResult = importResult
    }
}

public enum GitLabArchiveStoreError: Error, LocalizedError {
...
public enum GitLabArchiveStore {
    public static func requireName(_ raw: String?) throws -> String {
        guard let name = raw?.trimmingCharacters(in: .whitespacesAndNewlines), name.isEmpty == false else {
            throw GitLabArchiveStoreError.missingArchiveName
        }

        return name
    }

    public static func statuses(settings: GitLabArchiveSettings, name rawName: String? = nil) throws -> [GitLabArchiveSourceStatus] {
        let sources: [GitLabArchiveSource]
        if let rawName {
            let name = try self.requireName(rawName)
            sources = settings.gitlabArchivesSources(matching: name)
            if sources.isEmpty {
                throw GitLabArchiveStoreError.archiveNotFound(name)
            }
        } else {
    ...
    public static func status(for source: GitLabArchiveSource, fileManager: FileManager = .default) -> GitLabArchiveSourceStatus {
        ...
        return GitLabArchiveSourceStatus(
            id: source.id,
            name: source.name,
    ...
    public static func update(source: GitLabArchiveSource) throws -> GitLabArchiveUpdateResult {
        guard source.enabled else {
            throw GitLabArchiveStoreError.archiveDisabled(source.name)
        }
        ...
        if resolvedSource.localRepositoryPath?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty != false {
            guard resolvedSource.remoteURL?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false else {
                throw GitLabArchiveStoreError.missingSnapshotSource(resolvedSource.name)
            }
        ...
        guard let rawRepoPath = resolvedSource.localRepositoryPath?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            throw GitLabArchiveStoreError.missingRepositoryPath(resolvedSource.name)
        }
        ...
        return GitLabArchiveUpdateResult(source: resolvedSource, importResult: result)
    }
    ...
    public static func defaultSnapshotRepositoryPath(name: String, repositoryIdentifier: String? = nil) -> String {
        ...
        let base = FileManager.default
            ...
            .first?
            .appending(path: RepoPeekProductConstants.applicationSupportDirectoryName, directoryHint: .isDirectory)
            .appending(path: "Archives", directoryHint: .isDirectory)
            .appending(path: "\(fileName)-snapshot", directoryHint: .isDirectory)
            .path

        return base ?? "~/Library/Application Support/\(RepoPeekProductConstants.applicationSupportDirectoryName)/Archives/\(fileName)-snapshot"
    }
    ...
    public static func defaultDatabasePath(name: String, repositoryIdentifier: String? = nil) -> String {
        ...
        return base ?? "~/Library/Application Support/\(RepoPeekProductConstants.applicationSupportDirectoryName)/Archives/\(fileName).sqlite"
    }

    public static func source(repository rawRepository: String) -> GitLabArchiveSource? {
    ...
    public static func sameArchiveLocation(_ lhs: GitLabArchiveSource, _ rhs: GitLabArchiveSource) -> Bool {
    ...
    private static func updateSnapshotRepository(source: GitLabArchiveSource, repoPath: String) throws {
    ...
    private static func runGit(arguments: [String], workingDirectory: String?) throws {
            ...
            guard process.terminationStatus == 0 else {
                ...
                throw GitLabArchiveStoreError.gitFailed(message.isEmpty ? "git failed: \(arguments.joined(separator: " "))" : message)
            }

        #else
            throw GitLabArchiveStoreError.gitFailed("git is unavailable on this platform")
        #endif
    }
}
...
private extension GitLabArchiveSettings {
    func gitlabArchivesSources(matching name: String) -> [GitLabArchiveSource] {
