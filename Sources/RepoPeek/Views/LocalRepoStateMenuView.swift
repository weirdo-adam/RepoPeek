...
struct LocalRepoStateMenuView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            LocalRepoHeaderRow(status: self.status, language: self.language, isHighlighted: self.isHighlighted)
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline, spacing: MenuStyle.submenuIconSpacing) {
            ...
            if let worktreeName = self.status.worktreeName {
                Text(self.format("Worktree %@", worktreeName))
                    .font(.caption2)
                    .foregroundStyle(MenuHighlightStyle.secondary(self.isHighlighted))
            }
            Spacer(minLength: 8)
