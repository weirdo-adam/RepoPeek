...
struct RepoPeekNotification: Sendable {
    let identifier: String
    let title: String
    let body: String
    let url: URL?
    let clickAction: GitLabPullRequestNotificationClickAction
    let issueNavigatorMatch: GitLabReferenceMatch?

    init(
        identifier: String,
        title: String = RepoPeekProductConstants.displayName,
        body: String,
        url: URL? = nil,
        clickAction: GitLabPullRequestNotificationClickAction = .openInBrowser,
        issueNavigatorMatch: GitLabReferenceMatch? = nil
    ) {
...
// swiftformat:disable:next redundantSendable
enum RepoPeekNotificationClickTarget: Equatable, Sendable {
    case browser(URL)
    case issueNavigator([GitLabReferenceMatch])
    case none
}

actor RepoPeekNotifier {
    static let shared = RepoPeekNotifier()
    private let center: UNUserNotificationCenter?
    private let logger = RepoPeekLogging.logger("Notifications")
    ...
    func notify(_ notification: RepoPeekNotification) async {
    ...
    ) async -> RepoPeekNotificationDeliveryStatus {
        await withCheckedContinuation { continuation in
            center.getDeliveredNotifications { notifications in
                guard let notification = notifications.first(where: { $0.request.identifier == identifier }) else {
                    continuation.resume(returning: RepoPeekNotificationDeliveryStatus(
                        delivered: false,
                        urlAttached: false,
                        clickAction: "missing"
                    ))
...
// swiftformat:disable:next redundantSendable
private struct RepoPeekNotificationDeliveryStatus: Sendable {
    let delivered: Bool
    let urlAttached: Bool
    let clickAction: String
}

final class RepoPeekNotificationResponseHandler: NSObject, @unchecked Sendable, UNUserNotificationCenterDelegate {
    static let shared = RepoPeekNotificationResponseHandler()
    ...
    static func clickTarget(from userInfo: [AnyHashable: Any]) -> RepoPeekNotificationClickTarget {
        let clickAction = (userInfo[Self.clickActionUserInfoKey] as? String)
            .flatMap(GitLabPullRequestNotificationClickAction.init(rawValue:)) ?? .openInBrowser
    ...
    private static func issueNavigatorMatches(from userInfo: [AnyHashable: Any]) -> [GitLabReferenceMatch] {
...
private extension GitLabReferenceQuery {
    var issueNumber: Int? {
        switch self {
