...
@MainActor
final class GitLabReferenceMonitor {
    private let minimumBareDigits: Int
    private let pasteboard: NSPasteboard
    private let onPasteboardWithoutReference: () async -> Void
    private let onReferences: ([GitLabReferenceQuery], String) async -> Void
    private var pasteboardPoller: PasteboardTextPoller?
    private var isRunning = false

    init(
        minimumBareDigits: Int = AppLimits.GitLabReferenceMonitor.minimumBareDigits,
        pasteboard: NSPasteboard = .general,
        onPasteboardWithoutReference: @escaping () async -> Void = {},
        onReferences: @escaping ([GitLabReferenceQuery], String) async -> Void
    ) {
    ...
    static func query(from rawText: String, minimumBareDigits: Int = AppLimits.GitLabReferenceMonitor.minimumBareDigits) -> GitLabReferenceQuery? {
        GitLabReferenceTranslator.query(from: rawText, minimumBareDigits: minimumBareDigits)
    }

    static func queries(from rawText: String, minimumBareDigits: Int = AppLimits.GitLabReferenceMonitor.minimumBareDigits) -> [GitLabReferenceQuery] {
        GitLabReferenceTranslator.queries(from: rawText, minimumBareDigits: minimumBareDigits)
    }
}

private final class PasteboardTextPoller: @unchecked Sendable {
    private let pasteboard: NSPasteboard
    private let queue = DispatchQueue(label: "com.weirdoadam.repopeek.gitlab-reference-pasteboard", qos: .utility)
