...
struct RepositoryDisplayModel: Identifiable, Equatable {
    struct Stat: Identifiable, Equatable {
        ...
        init(id: String, label: String?, value: Int?, systemImage: String) {
            self.id = id
            self.label = label
            self.value = value
            self.valueText = value.map(StatValueFormatter.compact) ?? "--"
    ...
    init(repo: Repository, localStatus: LocalRepoStatus? = nil, now: Date = Date()) {
        ...
        self.stats = [
            ...
            Stat(id: "mrs", label: "MRs", value: Self.knownOpenPulls(from: repo), systemImage: "arrow.triangle.branch"),
    ...
    private static func knownOpenPulls(from repo: Repository) -> Int? {
        if repo.stats.openPulls > 0 { return repo.stats.openPulls }
        guard let freshness = repo.detailCacheState?.openPulls else { return nil }

        return freshness == .missing ? nil : repo.stats.openPulls
    }
}
