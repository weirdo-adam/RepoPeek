...
extension RecentListMenuCoordinator {
    ...
    func makeListItem(
        ...
        let item = NSMenuItem(title: self.t(title), action: action, keyEquivalent: "")
    ...
    func issueLabelMoreMenuItem(for options: [RecentIssueLabelOption]) -> NSMenuItem? {
        ...
        let all = NSMenuItem(
            title: self.t("All Labels"),
            action: #selector(StatusBarMenuManager.clearIssueLabelFilters),
            keyEquivalent: ""
        )
        ...
        let parent = NSMenuItem(title: self.t("More Labels…"), action: nil, keyEquivalent: "")
        parent.submenu = menu
        return parent
    }
    ...
    func addPullRequestMenuItem(_ summary: RepoPullRequestSummary, to menu: NSMenu) {
        let view = PullRequestMenuItemView(summary: summary, language: self.appState.session.settings.language) { [weak self] in
            self?.actionHandler.open(url: summary.url)
        }
        let item = self.menuItemFactory.makeItem(for: view, enabled: true, highlightable: true)
        item.toolTip = self.recentItemTooltip(title: summary.title, author: summary.authorLogin, updatedAt: summary.updatedAt)
    ...
    func addReleaseMenuItem(_ summary: RepoReleaseSummary, to menu: NSMenu) {
        ...
        let view = ReleaseMenuItemView(summary: summary, language: self.appState.session.settings.language) { [weak self] in
            self?.actionHandler.open(url: summary.url)
        }
    ...
    func addContributorMenuItem(_ summary: RepoContributorSummary, to menu: NSMenu) {
        ...
        let item = self.menuItemFactory.makeItem(for: view, enabled: true, highlightable: true)
        item.toolTip = "\(summary.login)\n\(self.format("%@ contributions", "\(summary.contributions)"))"
        menu.addItem(item)
    }
    ...
    func recentItemTooltip(title: String, author: String?, updatedAt: Date?) -> String {
        ...
        if let updatedAt {
            parts.append(self.format("Updated %@", RelativeFormatter.string(from: updatedAt, relativeTo: Date())))
        }
        parts.append(title)
