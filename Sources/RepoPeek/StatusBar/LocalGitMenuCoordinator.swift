...
final class LocalGitMenuCoordinator {
    ...
    func switchLocalBranch(_ sender: NSMenuItem) {
        ...
        self.runLocalGitTask(
            title: self.t("Switch branch failed"),
            status: nil,
            notifyOnSuccess: false,
    ...
    func createLocalBranch(_ sender: NSMenuItem) {
        ...
        let name = self.promptForText(
            ...
            message: self.t("Enter a new branch name.")
        )
        guard let name, name.isEmpty == false else { return }

        self.runLocalGitTask(
            title: self.t("Create branch failed"),
            status: nil,
            notifyOnSuccess: false,
    ...
    func createLocalWorktree(_ sender: NSMenuItem) {
        ...
        let branchName = self.promptForText(
            ...
            message: self.t("Enter a branch name for the new worktree.")
        )
        ...
        let pathText = self.promptForText(
            title: self.t("Worktree folder"),
            message: self.t("Enter the folder path for the new worktree."),
        ...
        do {
            ...
            self.presentAlert(title: self.t("Create worktree failed"), message: error.userFacingMessage)
            return
        }
        self.runLocalGitTask(
            title: self.t("Create worktree failed"),
            status: nil,
            notifyOnSuccess: false,
    ...
    private func refreshLocalBranchMenu(menu: NSMenu, entry: LocalGitMenuEntry) async {
        ...
        case let .success(snapshot):
            if snapshot.branches.isEmpty, snapshot.isDetachedHead == false {
                menu.addItem(self.menuBuilder.infoItem(self.t("No branches")))
                self.menuBuilder.refreshMenuViewHeights(in: menu)
                menu.update()
                return
            }
        ...
        case let .failure(error):
            menu.addItem(self.menuBuilder.infoItem(self.t("Failed to load branches")))
            self.presentAlert(title: self.t("Branch list failed"), message: error.userFacingMessage)
            self.menuBuilder.refreshMenuViewHeights(in: menu)
            menu.update()
        }
    }
    ...
    private func refreshCombinedBranchMenu(menu: NSMenu, entry: LocalGitMenuEntry) async {
        ...
        guard case .loggedIn = self.appState.session.account else {
            self.populateCombinedBranchMenu(
                ...
                remoteState: BranchesRemoteState(
                    branches: nil,
                    message: self.t("Sign in to load branches"),
                    emptyTitle: self.t("No branches")
                )
            )
            return
        }
        guard let (owner, name) = self.ownerAndName(from: fullName) else {
            self.populateCombinedBranchMenu(
                ...
                remoteState: BranchesRemoteState(
                    branches: nil,
                    message: self.t("Invalid repository name"),
                    emptyTitle: self.t("No branches")
                )
            )
            return
        ...
        let hostKey = entry.localStatus?.remoteWebURLHost.map { GitLabAccountSettings.hostKey(for: $0) }
        let cacheKey = hostKey.map { "\($0)/\(fullName)" } ?? fullName
        ...
        let remoteMessage = cachedBranches == nil && needsRefresh ? self.t("Loading…") : nil
        self.populateCombinedBranchMenu(
            menu: menu,
            entry: entry,
            localResult: localResult,
            remoteState: BranchesRemoteState(
                ...
                emptyTitle: self.t(descriptor.emptyTitle)
            )
        )
        ...
        do {
            let items = try await descriptor.load(cacheKey, hostKey, owner, name, self.recentMenuService.listLimit)
    ...
    private func populateCombinedBranchMenu(
        ...
        case let .success(snapshot):
            if snapshot.branches.isEmpty, snapshot.isDetachedHead == false {
                menu.addItem(self.menuBuilder.infoItem(self.t("No local branches")))
            }
            if snapshot.isDetachedHead {
                let model = LocalRefMenuRowViewModel(
                    kind: .branch,
        ...
        case let .failure(error):
            menu.addItem(self.menuBuilder.infoItem(self.t("Failed to load local branches")))
            self.presentAlert(title: self.t("Branch list failed"), message: error.userFacingMessage)
        }
        ...
        if let remoteMessage = remoteState.message {
            ...
            if remoteBranches.isEmpty {
                ...
                for branch in remoteBranches {
                    self.addRemoteBranchMenuItem(branch, repoFullName: entry.fullName, localStatus: entry.localStatus, to: menu)
                }
            }
        } else {
            menu.addItem(self.menuBuilder.infoItem(remoteState.emptyTitle))
    ...
    private func refreshLocalWorktreeMenu(menu: NSMenu, entry: LocalGitMenuEntry) async {
        ...
        case let .success(worktrees):
            if worktrees.isEmpty {
                menu.addItem(self.menuBuilder.infoItem(self.t("No worktrees")))
                self.menuBuilder.refreshMenuViewHeights(in: menu)
                menu.update()
                return
            }
            for worktree in worktrees {
                let branch = worktree.branch ?? self.t("Detached")
            ...
            menu.update()
        case let .failure(error):
            menu.addItem(self.menuBuilder.infoItem(self.t("Failed to load worktrees")))
            self.presentAlert(title: self.t("Worktree list failed"), message: error.userFacingMessage)
            self.menuBuilder.refreshMenuViewHeights(in: menu)
            menu.update()
        }
    }
    ...
    private func makeLocalWorktreeMenuItem(
        ...
        let row = LocalWorktreeMenuRowView(model: model, language: self.appState.session.settings.language)
    ...
    private func makeLocalBranchMenuItem(
        ...
        let row = LocalBranchMenuRowView(model: model, language: self.appState.session.settings.language)
        let item = self.menuItemFactory.makeItem(for: row, enabled: true, highlightable: true)
        item.target = self.actionHandler
    ...
    private func addRemoteBranchMenuItem(
        _ summary: RepoBranchSummary,
        repoFullName: String,
        ...
        let view = BranchMenuItemView(summary: summary) { [weak self] in
            ...
            let webURLBuilder = localStatus?.remoteWebURLHost.map(RepoWebURLBuilder.init(host:)) ?? self.webURLBuilder
            guard let url = webURLBuilder.branchURL(fullName: repoFullName, branch: summary.name) else { return }
    ...
    private func promptForText(title: String, message: String, defaultValue: String? = nil) -> String? {
        ...
        alert.addButton(withTitle: self.t("Cancel"))
    ...
    private func switchLocalWorktree(path: URL, fullName: String) {
        ...
        guard FileManager.default.fileExists(atPath: pathString) else {
            self.presentAlert(
                title: self.t("Worktree missing"),
                message: self.format("Could not find %@.", pathString)
            )
            return
        }
    ...
    private func confirmHardReset(for status: LocalRepoStatus) -> Bool {
        ...
        alert.messageText = self.format("Hard reset %@?", status.displayName)
        alert.informativeText = self.format(
            "This will discard uncommitted changes and reset to %@.",
            status.upstreamBranch ?? "upstream"
        )
        alert.addButton(withTitle: self.t("Reset"))
        alert.addButton(withTitle: self.t("Cancel"))
