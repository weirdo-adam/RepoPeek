...
final class RecentListMenuCoordinator {
    ...
    private let logger = RepoPeekLogging.logger("recent-list")
    ...
    func cachedRecentCommitCount(cacheKey: String) -> Int? {
        self.menuService.cachedRecentCommitCount(cacheKey: cacheKey)
    }
    ...
    func prefetchRecentLists(fullNames: Set<String>) {
        ...
        for fullName in fullNames {
            self.prefetchRecentLists(fullName: fullName, hostKey: nil)
        }
    }

    func prefetchRecentLists(fullName: String, hostKey: String?) {
        guard case .loggedIn = self.appState.session.account else { return }
        guard fullName.isEmpty == false else { return }

        let kinds = self.menuService.descriptors().keys
        for kind in kinds {
            self.prefetchRecentList(fullName: fullName, hostKey: hostKey, kind: kind)
        }
    }

    func prefetchRecentList(fullName: String, hostKey: String? = nil, kind: RepoRecentMenuKind) {
        ...
        let cacheKey = RepoRecentMenuContext.cacheKey(fullName: fullName, hostKey: hostKey)
        ...
        Task { @MainActor [weak self] in
            ...
            do {
                ...
                _ = try await descriptor.load(cacheKey, hostKey, owner, name, self.menuService.listLimit)
    ...
    private func refreshRecentListMenu(menu: NSMenu, context: RepoRecentMenuContext) async {
        ...
        let headerActions = self.headerActions(for: context.kind, fullName: context.fullName)
        ...
        if let stale {
            ...
            self.populateRecentListMenu(menu, header: header, actions: headerActions, extras: staleExtras, content: .loading)
        }
        menu.update()
        ...
        do {
            let items = try await descriptor.load(cacheKey, context.hostKey, owner, name, self.menuService.listLimit)
            self.logger.info(
    ...
    func timeoutMessage(timeout: TimeInterval) -> String {
        self.format("Timed out after %@s", "\(Int(timeout.rounded()))")
    }
    ...
    func failureMessage(for error: Error) -> String {
        ...
        guard !message.isEmpty else { return self.t("Failed to load") }

        return self.format("Failed: %@", message)
    }
    ...
    static func rateLimitMessage(for _: Error) -> String? {
        nil
    }
    ...
    private func recordRateLimitIfNeeded(_: Error) {}
    ...
    private func renderRecentItems(_ items: RecentMenuItems, for _: RepoRecentMenuKind, repoFullName: String, menu: NSMenu) {
        ...
        case let .pullRequests(pullRequestItems):
            ...
            if filtered.isEmpty {
                self.addEmptyListItem("No matching merge requests", to: menu)
                return
            }
    ...
    private func headerActions(for kind: RepoRecentMenuKind, fullName: String) -> [RecentMenuAction] {
        switch kind {
        case .releases:
    ...
    private func openAction(for kind: RepoRecentMenuKind) -> Selector {
        ...
        case .ciRuns: #selector(StatusBarMenuManager.openPipelines(_:))
