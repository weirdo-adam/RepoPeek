...
struct VisibilityTests {
    ...
    func `pinRepository removes the repo from the hidden list`() {
        ...
        let changed = settings.pinRepository("owner/X", forAccountID: nil)

        #expect(changed)
        #expect(settings.pinnedRepositories.contains { $0.caseInsensitiveCompare("owner/X") == .orderedSame })
        #expect(!settings.hiddenRepositories.contains { $0.caseInsensitiveCompare("owner/X") == .orderedSame })
    }
    ...
    func `pinRepository repairs an already pinned repo that is still hidden`() {
        ...
        #expect(changed)
        #expect(settings.pinnedRepositories.count(where: { $0.caseInsensitiveCompare("owner/X") == .orderedSame }) == 1)
        #expect(!settings.hiddenRepositories.contains { $0.caseInsensitiveCompare("owner/X") == .orderedSame })
    }
    ...
    @Test
    func `hides repositories under hidden groups`() {
        let repos = [
            makeRepository(id: "1", name: "api", owner: "example/product"),
            makeRepository(id: "2", name: "ui", owner: "example/product/web"),
            makeRepository(id: "3", name: "ops", owner: "example/devops")
        ]
        let visible = AppState.selectVisible(
            all: repos,
            options: AppState.VisibleSelectionOptions(
                pinned: ["example/product/api"],
                hidden: [],
        ...
        #expect(visible.map(\.fullName) == ["example/devops/ops"])
    }
}
