...
struct LocalProjectsServiceTests {
    ...
    func `snapshot discovers repos and parses remote formats`() async throws {
        ...
        try initializeRepo(at: repoA, origin: "git@gitlab.com:foo/repo-a.git")
        try initializeRepo(at: repoB, origin: "https://gitlab.com/foo/repo-b.git")
    ...
    @Test
    func `snapshot preserves GitLab subgroup remotes and web hosts`() async throws {
        let root = try makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: root) }

        let sshRepo = root.appendingPathComponent("ssh-repo", isDirectory: true)
        let httpsRepo = root.appendingPathComponent("https-repo", isDirectory: true)
        try FileManager.default.createDirectory(at: sshRepo, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: httpsRepo, withIntermediateDirectories: true)

        try initializeRepo(at: sshRepo, origin: "git@gitlab.com:group/subgroup/ssh-repo.git")
        try initializeRepo(at: httpsRepo, origin: "https://gitlab.example.com/root/platform/https-repo.git")
        ...
        let sshStatus = snapshot.statuses.first(where: { $0.name == "ssh-repo" })
        #expect(sshStatus?.fullName == "group/subgroup/ssh-repo")
        #expect(sshStatus?.displayName == "group/subgroup/ssh-repo")
        #expect(sshStatus?.remoteWebURLHost?.absoluteString == "https://gitlab.com")

        let httpsStatus = snapshot.statuses.first(where: { $0.name == "https-repo" })
        #expect(httpsStatus?.fullName == "root/platform/https-repo")
        #expect(httpsStatus?.remoteWebURLHost?.absoluteString == "https://gitlab.example.com")
    }
    ...
    func `discover repo roots accepts file reference UR ls`() throws {
        ...
        try initializeRepo(at: repo, origin: "git@gitlab.com:foo/repo-a.git")
    ...
    func `discover repo roots follows symlinked directories once`() throws {
        ...
        try initializeRepo(at: targetRepo, origin: "git@gitlab.com:foo/target-repo.git")
    ...
    func `local repo index matches case insensitive names and full names`() {
        let status = LocalRepoStatus(
            path: URL(fileURLWithPath: "/tmp/LocalApp"),
            name: "LocalApp",
            fullName: "example/LocalApp",
            branch: "main",
            isClean: true,
            aheadCount: 0,
            behindCount: 0,
        ...
        let repo = makeRepository(name: "localapp", owner: "example")

        #expect(index.status(for: repo) != nil)
        #expect(index.status(forFullName: "EXAMPLE/LOCALAPP") != nil)
    }
    ...
    func `snapshot includes worktree name for worktrees`() async throws {
        ...
        try FileManager.default.createDirectory(at: repo, withIntermediateDirectories: true)
        try initializeRepo(at: repo, origin: "git@gitlab.com:foo/repo.git")
...
private func makeTempDirectory() throws -> URL {
    let url = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("repopeek-localprojects-\(UUID().uuidString)", isDirectory: true)
